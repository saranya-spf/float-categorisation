...
def prepare_ml_data(df: pd.DataFrame, test_size: float = 0.2):
    ...
    return X_train, X_test, y_train, y_test, df.index, label_names
...
def inference_bert_models(
    ...
    unmatched_X = unmatched.loc[:, "aggregated_text"]
...
def evaluate_bert_models(
    ...
    _, all_preds = trainer.predict(model, model_dict_path, X)
